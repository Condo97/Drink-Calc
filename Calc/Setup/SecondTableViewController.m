...
- (void) viewWillAppear:(BOOL)animated {
    ...
    [self.continueButtonOutlet setEnabled:NO];
                ...
                [[HealthKitManager sharedManager] getWeightWithCompletion:^(BOOL successful, double value){
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if(value != 0) {
                            ...
                            if(self.weight != 0 && (self.feet != 0 || self.inches != 0) && self.birthdate != 0)
                                [self.continueButtonOutlet setEnabled:YES];
                            else
                                [self.continueButtonOutlet setEnabled:NO];
                        }
                    });
...
- (IBAction) continueButton:(id)sender {
    if(self.weight != 0 && (self.feet != 0 || self.inches != 0) && self.birthdate != nil) {
        ...
        NSArray *ringNamesInOrder = [[JSONManager sharedManager] getRingNamesInOrderWithJSONDictionary:ringsJson];
        NSMutableDictionary *usesAutoLimitDictionary = [[NSMutableDictionary alloc] init];
        
        for(NSString *ringName in ringNamesInOrder)
            [usesAutoLimitDictionary setObject:@YES forKey:ringName];
        ...
        [[NSUserDefaults standardUserDefaults] setObject:usesAutoLimitDictionary forKey:@"ringAutoLimitDictionary"];
        
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"initialSetupComplete"
         ];
        
        [self performSegueWithIdentifier:@"toFinalSetupView" sender:nil];
    }
}
