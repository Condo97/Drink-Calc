...
@IBDesignable
@objc public class KDCircularProgress: UIView, CAAnimationDelegate {
    ...
    @objc public var progressColors: [UIColor] {
        get {
    ...
    @objc public func animate(fromAngle: Double, toAngle: Double, duration: TimeInterval, relativeDuration: Bool = true, completion: ((Bool) -> Void)?) {
        if isAnimating() {
            pauseAnimation()
        }
    ...
    @objc public func animate(toAngle: Double, duration: TimeInterval, relativeDuration: Bool = true, completion: ((Bool) -> Void)?) {
        if isAnimating() {
            pauseAnimation()
        }
    ...
    private class KDCircularProgressViewLayer: CALayer {
        ...
        @objc var roundedCorners: Bool = true
        @objc var lerpColorMode: Bool = false
        @objc var gradientRotateSpeed: CGFloat = 0 {
            didSet {
                invalidateGradientCache()
            }
        }
        @objc var glowAmount: CGFloat = 0
        var glowMode: KDCircularProgressGlowMode = .forward
        @objc var progressThickness: CGFloat = 0.5
        @objc var trackThickness: CGFloat = 0.5
        @objc var trackColor: UIColor = .black
        @objc var progressInsideFillColor: UIColor = .clear
